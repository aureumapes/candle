...
func Add(args []string, vars map[string]string) map[string]string {
	if _, varExists := vars[args[0]]; varExists {
		if util.IsNumeric(vars[args[0]]) {
			if util.IsNumeric(args[1]) {
				...
				convertedAddition, _ := strconv.ParseFloat(args[1], 64)
				vars[args[0]] = strconv.FormatFloat(convertedVar+convertedAddition, 'f', -1, 64)
			} else if _, varExists = vars[args[1]]; varExists {
				convertedVar, _ := strconv.ParseFloat(vars[args[0]], 64)
				convertedAddition, _ := strconv.ParseFloat(vars[args[1]], 64)
				vars[args[0]] = strconv.FormatFloat(convertedVar+convertedAddition, 'f', -1, 64)
			} else {
